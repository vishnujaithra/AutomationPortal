...
namespace OhpnmAutomation.Pages.Registration
{
    public class ProfessionalLicense : BasePage
    {
        ...
        public IWebElement FileControl
        {
            get
            {
                return PageHelper.FindElement(webDriver, By.Id($"ctl00_MainContent_ucLicenses_{RegID}_125_RadAsyncUpload1file0"), null);
            }
        }
        public By FileDelete
        {
            get
            {
                return By.Id($"ctl00_MainContent_ucLicenses_{RegID}_125_LnkButtonDelete");
            }
        }
        ...
        public void SetProfessionalLicense(ProfessionalLicenses info)
        {
            ...
            IWebElement fileElem =  FileControl;

            ((IJavaScriptExecutor)WebDriver).ExecuteScript("arguments[0].scrollIntoView(true);", fileElem);


            if (fileElem != null)
            {
                fileElem.SendKeys(info.FilePath);
                Thread.Sleep(4000);
            }

            while (!PageHelper.IsElementIsVisible(WebDriver, FileDelete, TimeoutConfiguration.Element))
            {
                Thread.Sleep(500);
            }
        }
